...
fun MapView(
    ...
    BoxWithConstraints(modifier = mapModifier.background(Color(0xFFF8FAFC))) {
        ...
        Canvas(
            ...
            withTransform({
                ...
                // LAYER 4: Navigation Path
                navState.path?.let { path ->
                    if (path.size > 1 && navState.isNavigatingMode) {
 ...
 // Outer glow
 drawCircle(Color(0xFFFF1744).copy(0.3f), radius = 20f / scale, center = destPos)
 // Red circle
 drawCircle(Color(0xFFFF1744), radius = 10f / scale, center = destPos)
 // White inner dot
 drawCircle(Color.White, radius = 5f / scale, center = destPos)
